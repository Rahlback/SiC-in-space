...
void test(void)
{
	...
	/* Receive EXP_SEND header */
	r = simulate_loop(&test_link);
	test_assert(test_link.transaction_id == 0, "Transaction-ID be set to 0");
	test_assert(test_link.frame_id == 0, "Frame-ID should be set to 0");
	test_assert(test_link.total_length == 260, "total_length of the transaction should be set in the link");
	...
	r = simulate_loop(&test_link);
	test_assert(test_link.transaction_id == 0, "");
	test_assert(test_link.frame_id == 1, "Frame-ID should have been updated");
	test_assert(r.status == MSP_RESPONSE_OK, "");
	DEBUG_EXEC(if (r.status != MSP_RESPONSE_OK) print_response(r));
