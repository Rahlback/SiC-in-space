...
void test(void)
{
	...
	msp_seqflags_set(&test_link.flags, MSP_OP_SLEEP, 1);

	r = msp_start_transaction(&test_link, MSP_OP_SLEEP, 0);
	...
	r = simulate_loop(&test_link);
	test_assert(r.status == MSP_RESPONSE_TRANSACTION_SUCCESSFUL, "Should have a T_ACK");
	...
	/* Make sure that two frames were sent */
...
int msp_i2c_write(unsigned long slave_address, unsigned char *data, unsigned long size)
{
	...
	switch (seq) {
	case 0:
		test_assert(data[0] == MSP_OP_SLEEP, "");
