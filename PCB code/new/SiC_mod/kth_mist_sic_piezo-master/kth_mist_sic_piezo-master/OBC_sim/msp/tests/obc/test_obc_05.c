...
void test(void)
{
	...
	/* Receive Response */
	r = simulate_loop(&test_link);
	test_assert(test_link.transaction_id == 0, "");
	...
	/* Receive data frame (it should be corrupted) */
	r = simulate_loop(&test_link);
	...
	/* Make sure that 7 frames were sent */
	test_assert(seq == 7, "7 frames should've been sent");

	/* Check that the received data is correct */
	is_correct = 1;
	for (i = 0; i < 30; i++) {
		if (test_storage[i] != 0xAA)
			is_correct = 0;
	}
...
int msp_i2c_write(unsigned long slave_address, unsigned char *data, unsigned long size)
{
	...
	case 2: /* Ack response frame */
		...
		test_assert(fcs == msp_from_bigendian32(data + 5), "");
		break;
	case 4: /* Ack the response frame again! */
		test_assert(data[0] == MSP_OP_F_ACK, "");
		test_assert(msp_from_bigendian32(data + 1) == 0, "DL = 0");
		fcs = msp_crc32(data, 5, fcs);
		test_assert(fcs == msp_from_bigendian32(data + 5), "");
		break;
	case 6: /* Ack the transaction */
...
int msp_i2c_read(unsigned long slave_address, unsigned char *data, unsigned long size)
{
	...
	case 1:
		...
		msp_to_bigendian32(data + 1, 30); /* DL = 30 */
		fcs = msp_crc32(data, 5, fcs);
		msp_to_bigendian32(data + 5, fcs);
		break;
	case 3:
		test_assert(size == (30 + 5), "length of data frame");

		data[0] = MSP_OP_DATA_FRAME | 0x80;
		for (i = 0; i < 30; i++)
			data[i + 1] = 0x11;
		fcs = msp_crc32(data, 30 + 1, fcs);
		msp_to_bigendian32(data + 30 + 1, fcs);

		/* Now we flip a bit to mess up the FCS check. This means that the data
		 * that we successfully receive at the end should not be 0x11, but
		 * instead 0xAA. */
		data[23] ^= 0x04;
		break;
	case 5:
